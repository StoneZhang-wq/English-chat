...
document.addEventListener("DOMContentLoaded", function() {
    ...
    async function startSession() {
        try {
            ...
            const data = await BaseApi.api('/openai_ephemeral_key');
            ...
            try {
                const sdpResponse = await BaseApi.api(proxyUrl, {
                    method: "POST",
                    body: sdp,
                    headers: {
    ...
    async function fetchCharacterPrompt(characterName) {
        ...
        try {
            const data = await BaseApi.api(`/api/character/${characterName}`);
            if (data.error) {
