document.addEventListener("DOMContentLoaded", function() {
    ...
    params.set('channel', 'scripts');
    try {
        const uid = typeof sessionStorage !== 'undefined' && sessionStorage.getItem('current_user_id');
        if (uid) params.set('user_id', uid);
    } catch (e) {}
    const websocket = new WebSocket(`${protocol}//${host}/ws?${params.toString()}`);
    ...
    async function fetchOllamaModels() {
        try {
            const data = await BaseApi.api('/ollama_models');
    ...
    clearButton.addEventListener('click', async function() {
        messages.innerHTML = '';
        try {
            await BaseApi.api('/clear_history', { method: 'POST' });
    ...
    transcriptionSelect.addEventListener('change', function() {
        BaseApi.api('/set_transcription_model', {
            method: 'POST',
    ...
    async function downloadHistory() {
        const historyText = await BaseApi.api('/download_history');
        if (historyText) {
    ...
    async function fetchKokoroVoices() {
        try {
            const data = await BaseApi.api('/kokoro_voices');
