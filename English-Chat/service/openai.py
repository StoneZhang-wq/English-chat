...
class OpenAIService:
    def __init__(self):
        ...
        self.model_name: str = configer.openai_model or "gpt-3.5-turbo"
        self.timeout = 60
    ...
    async def generate_text(self, prompt: str, system_prompt="You are a helpful assistant.") -> str:
        if not self.api_key:
    ...
    async def generate_chat(self, message: str, context_msg: List[Dict[str, str]], system_prompt="You are a helpful assistant.") -> str:
        if not self.api_key:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="OPENAI_API_KEY not set")
        ...
        # 添加当前用户消息；设置 max_tokens 避免回复被截断（如 generate-review 只返回 "corrections:"）
        ...
        try:
            ...
            logger.error(f"OpenAI chat completion error: {str(e)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="OpenAI chat completion error, please try again")
